...
with st.expander("⚙️ Painel de Modernização — Nova Efetivação"):
    '''
    **Painel desenvolvido para acompanhamento diário dos principais indicadores do Nova Efetivação, proporcionando uma visão clara e atualizada dos resultados da modernização do CD.**
    '''
...
with st.expander("🧠 Modelo de Machine Learning para Classificação"):
    ...
    st.markdown("""
**Estudo de reclamações de débitos usando técnicas de NLP e regressão logística para clusterizaçção e classificação de problemas relacionados à plataforma de débitos.**

### Abordagem
- Clusterização de reclamações via NLP
- Validação de clusters e classificação Plataforma vs Produto
- APlicação de regressão logística para a base

### Benefícios 
- **Separação** entre **problemas da plataforma Vs questões de produto**
- **Visão clara** sobre a **origem** das reclamações relacionadas à débito
- **Otimização de tempo** e **reduçção de trabalho** na análise de dores
""")